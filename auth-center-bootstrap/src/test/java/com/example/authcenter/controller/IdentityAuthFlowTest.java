...
import com.example.authcenter.identity.usecase.impl.AuthenticateUseCaseImpl;
import com.example.authcenter.identity.usecase.impl.LoginUseCaseImpl;
...
import com.example.authcenter.infra.identity.service.JwtIdentityTokenProvider;
...
class IdentityAuthFlowTest {
    ...
    void loginLogoutFlowShouldCreateAndInvalidateSessionBackedToken() throws Exception {
        ...
        IdentityAccount account = identityAccountFactory.restore(
                1L,
                username,
                passwordHasher.encode(new com.example.authcenter.domain.identity.model.valueobject.RawPassword(password)),
                "tester@example.com"
        );
    ...
    void openApiJsonShouldBeAccessibleWithoutAuthentication() throws Exception {
        mockMvc.perform(get("/v3/api-docs"))
                ...
                .andExpect(jsonPath("$.paths['/auth/login'].post.summary").value("账号登录"))
                .andExpect(jsonPath("$.paths['/auth/register'].post.summary").value("注册账号"))
                .andExpect(jsonPath("$.paths['/auth/update-password'].post.summary").value("修改密码"))
                .andExpect(jsonPath("$.components.securitySchemes.bearerAuth.scheme").value("bearer"))
                .andExpect(jsonPath("$.paths['/auth/logout'].post.security").isArray())
                .andExpect(jsonPath("$.paths['/auth/login'].post.requestBody").exists())
                .andExpect(jsonPath("$.paths['/auth/update-password'].post.requestBody").exists())
